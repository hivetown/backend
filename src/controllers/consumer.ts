...
export class ConsumerController {
	...
	public async addCartItem(
		...
		try {
			const consumer = await container.consumerGateway.findByIdWithCart(consumerId);
			const product = await container.producerProductGateway.findById(Number(req.body.producerProduct));
			if (consumer) {
				if (product) {
	...
	public async updateQuantityCartItem(
		...
		try {
			...
			if (consumer) {
				const product = await container.producerProductGateway.findById(Number(producerProductId));
